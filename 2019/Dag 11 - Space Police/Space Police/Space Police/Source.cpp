...
int main() {
	...
	cout << "\nHär är vilka som blivit målade: ";
	...
	for (int x = 0; x < controlGrid.maxX; x++)
		for (int y = 0; y < controlGrid.maxY; y++)
			if (controlGrid.grid[x][y])
				numberOfPainted++;
	cout << "\nTotalt målades alltså, minst en gång: " << numberOfPainted << " st plattor";
...
bool* gridValue(boolGrid* grid, int targetX, int targetY) {//Skickar tillbaka värdet med koordinaterna [targetX][targetY]
...
void movePainterBot(painterBot* painter, int instruction) {
	if (instruction == 0)//Ska svänga åt vänster
		...
		cout << "ERROR ROBOT SVÄNGER OLAGLIGT";
