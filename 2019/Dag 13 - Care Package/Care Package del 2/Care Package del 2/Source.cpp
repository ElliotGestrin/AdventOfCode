...
void IntcodeComputer(state*, int input, intGrid* grid);
...
int main() {
	...
	IntcodeComputer(&computer, NAN, &grid);

	//displayGrid(&grid);
	antalBlock = howManyGrid(&grid, 2);
	cout << "\nDet finns totalt: " << antalBlock << ", dags at spela";

	computer.lista[0] = 2;
	computer.trueEnd = false;
	computer.i = 0;
	computer.relativeBase = 0;
		...
		if (computer.trueEnd != true) {
			cout << "ENTERING";
			if (computer.output1 == -1 and computer.output2 == 0) {
				currentScore = computer.output3;
				cout << endl << "Score: " << currentScore;
			}
			else {
...
void IntcodeComputer(state* nuvarande, int input, intGrid* grid) {
	int i = nuvarande->i;
	...
	while (lista[i] != 99) {//Kontrollerar först om det ska va slut, genomför sedan de faktiska instruktionerna. Adderar fyra då det är förskjutningen mellan de olika "kommandona" (kommandot och tre argument)
		...
		int currentScore = 0; //ENBART FÖR SPELET
		...
		case 4: //Output
			...
			if(isnan(nuvarande->output1))
				...
				if (nuvarande->output1 == -1 and nuvarande->output2 == 0) {
					currentScore = nuvarande->output3;
					cout << endl << "Score: " << currentScore;
				}
				else {
					changeGrid(&grid, nuvarande->output1, nuvarande->output2, nuvarande->output3);

					nuvarande->output1 = NAN;
					nuvarande->output2 = NAN;
					nuvarande->output3 = NAN;
				}
